...
class Input{
  ...
  down(e){
    ...
    if(this._mode==='r1'){
      ...
      else { let pan=0; for(const q of this.pointers.values()) if(q.role==='pan') pan++; p.role=pan?'pan':'act'; }
      this.pointers.set(p.id,p); try{ this.canvas.setPointerCapture(p.id); }catch(err){}
      if(p.role==='act') this.onPress&&this.onPress(p.x,p.y,p.type,p.id);
      return;
    }
  ...
  up(e){
    ...
    if(this._mode==='r1'){ if(p.role==='act') this.onRelease&&this.onRelease(p.id); return; }
