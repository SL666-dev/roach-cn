...
(async()=>{
 ...
 await page.screenshot({path:path.join(root,'tools/out/check-game.png')});
 ...
 await page.screenshot({path:path.join(root,'tools/out/check-lab.png')});
 ...
 await page.screenshot({path:path.join(root,'tools/out/check-mobile.png')});
 ...
 fs.writeFileSync(path.join(root,'tools/out/check-errors.json'),JSON.stringify(errors,null,2));
